...
interface TransactionDao {
    ...
    @Query(
        ...
        LEFT JOIN categories c ON c.id = t.categoryId
        LEFT JOIN fiscal_documents fd ON fd.linkedTransactionId = t.id
        WHERE (t.accountId = :accountId OR t.transferToAccountId = :accountId) AND t.isIgnored = 0
        ORDER BY t.date ASC, t.createdAt ASC
        """,
    )
    fun observeStatementForAccount(accountId: Long): Flow<List<TransactionWithDetails>>
